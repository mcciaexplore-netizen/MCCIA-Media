...
export type PublicWorkItem={id:string;title:string;category:Category;stage:string;publicationDate:string;completed:boolean};
export type WorkItem={publicSnapshot?:PublicWorkItem;publicApprovedAt?:string;publicApprovedBy?:string;reviewer?:string;priority?:string;publicationDate?:string;issueMonth?:string;theme?:string;acknowledgementDate?:string;followUpDate?:string;quantity?:string;deliveryMethod?:string;receiptDate?:string;resolutionNote?:string;documents?:DocumentVersion[];sourceConfirmed?:boolean;id:string;category:Category;title:string;stage:string;owner:string;dueDate:string;nextAction:string;notes:string;workingUrl:string;publishedUrl:string;recipient:string;sentDate:string;response:string;printStatus:string;websiteStatus:string;distributionStatus:string;flags:string[];source:{sheet:string;row:number;cells:Record<string,string>}|null;revision:number;updatedAt:string;history:{at:string;summary:string;actor?:string;comment?:string;fromStage?:string;toStage?:string}[]};
...
// Explicit allowlist: never spread a private work item into a public response.
export function publicSnapshot(item:WorkItem):PublicWorkItem{return {id:item.id,title:item.title,category:item.category,stage:item.stage,publicationDate:item.publicationDate||'',completed:complete(item)}}
export function approvedPublicItems(items:WorkItem[]):PublicWorkItem[]{return items.filter(x=>x.publicApprovedAt&&x.publicSnapshot).map(x=>{const p=x.publicSnapshot!;return {id:x.id,title:p.title,category:p.category,stage:p.stage,publicationDate:p.publicationDate,completed:p.completed}})}
