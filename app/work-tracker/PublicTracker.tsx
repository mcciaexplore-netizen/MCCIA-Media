...
export default function PublicTracker(){
 ...
 useEffect(()=>{fetch('/api/work-tracker/public',{cache:'no-store'}).then(async r=>{const data=await r.json();if(!r.ok)throw Error(data.error||'Tracker unavailable');setItems(data.items)}).catch(e=>setError(e.message)).finally(()=>setLoading(false))},[]);
