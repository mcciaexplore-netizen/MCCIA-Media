...
export function zohoConfig(){
 ...
 const u=new URL(redirect);const allowed=process.env.NODE_ENV==='development'?['localhost','127.0.0.1'].includes(u.hostname):u.origin==='https://mccia-media.vercel.app';if(!allowed||(process.env.NODE_ENV!=='development'&&u.protocol!=='https:')||u.pathname!=='/api/work-tracker/zoho/callback'||u.search||u.hash||u.username||u.password)throw Error('Configure the correct tracker callback URL.');
 const members=JSON.parse(process.env.ZOHO_TEAM_ROLES||'{}') as Record<string,Role>;
 if(!members||typeof members!=='object'||Array.isArray(members)||Object.entries(members).some(([email,role])=>! /^[a-z0-9]+(?:[._%+-][a-z0-9]+)*@mcciapune\.com$/.test(email)||!roles.includes(role)))throw Error('Configure approved MCCIA emails and roles.');
 return {client,secret,redirect,host,members,ready:Boolean(client&&secret&&(Object.keys(members).length||process.env.WT_INITIAL_ADMIN))};
}
...
export function verifyZohoToken(token:string,keys:JsonWebKey[],client:string,issuer:string,nonce:string){
 ...
 if((c.iss!==issuer&&c.iss!==new URL(issuer).hostname)||c.aud!==client||(c.azp&&c.azp!==client)||typeof c.exp!=='number'||c.exp<=now||typeof c.iat!=='number'||c.iat>now+60||c.nonce!==nonce||c.email_verified!==true||typeof c.sub!=='string'||!c.sub||typeof c.email!=='string')throw Error('Identity could not be verified.');
