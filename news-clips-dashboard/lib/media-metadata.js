...
export const LANGUAGES = ['English', 'Marathi', 'Hindi', 'Marathi / Hindi', 'English / Marathi', 'Marathi / Hindi / English', 'Multilingual', 'Other', LANGUAGE_NOT_RECORDED];
...
export function validPublicationDate(value, today = indiaToday()) {
    const candidate = calendarDate(value);
    return candidate && candidate <= today ? candidate : null;
}
export function publicationYear(item, today = indiaToday()) {
...
export function comparePublicationDates(a, b, oldest = false) {
...
export function normalizeRecordMetadata(item) {
...
export function normalizeLanguage(value) {
    const candidate = String(value ?? '').trim();
    const aliases = { en: 'English', eng: 'English', mr: 'Marathi', mar: 'Marathi', hi: 'Hindi', hin: 'Hindi' };
...
export function recordLookup(records) {
    const result = new Map();
    for (const record of records) {
        if (record.id)
            result.set(record.id, record);
        for (const id of record.mergedRecordIds ?? [])
            result.set(id, record);
    }
    return result;
}
export function resolveClippingMetadata(item, record) {
    if (record && item.metadataSource?.startsWith('Connected record '))
        return { ...item, year: publicationYear(item), language: normalizeLanguage(record.language), presence: record.presence || PERSON_NOT_RECORDED };
...
export function uniqueCoverage(records, clippings) {
...
export function yearCounts(items, today = indiaToday()) {
...
export function peopleCategory(presence) {
    ...
    if (/^both\b/.test(value) || (dg && president))
        return 'DG and MCCIA President';
    if (dg)
        return 'Director General / Prashant Girbane';
    if (president)
        return 'MCCIA President';
    if (/mccia/.test(value))
        return 'MCCIA / person not specified';
    if (value === 'kopardekar')
        return 'Sudhanwa Kopardekar';
