...
export function normalizeDgEngagementType(value) {
    const candidate = String(value ?? '').trim();
    return DG_ENGAGEMENT_TYPES.find((item) => item === candidate) ?? null;
}
export function mentionsDg(value) {
    const text = String(value ?? '').toLowerCase();
    return DG_MARKERS.some((marker) => text.includes(marker));
}
export function inferDgEngagementType(value) {
    ...
    if (authored.some((pattern) => pattern.test(text)))
        return DG_ENGAGEMENT_TYPES[0];
    const conversation = [
    ...
    if (conversation.some((pattern) => pattern.test(text)))
...
export function resolveRecordDgEngagementType(record) {
    ...
    if (/\b(?:authored|co-authored)(?:\b|;)/.test(presence))
        return DG_ENGAGEMENT_TYPES[0];
    ...
    if (presence.includes('named or pictured in clipping'))
        return null;
