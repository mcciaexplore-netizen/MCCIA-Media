...
test('all e-paper dates retain precision and report freshness includes data changes',()=>{const epapers=JSON.parse(fs.readFileSync(new URL('../app/epaper-sources.json',import.meta.url)));assert.ok(epapers.length>=64);const monthly=epapers.filter(r=>r.datePrecision==='month');assert.ok(monthly.length>=62);for(const r of monthly)assert.ok(monthRows(epapers,r.publicationMonth).some(x=>x.id===r.id));assert.notEqual(reportRevision([{id:'A',title:'Old'}]),reportRevision([{id:'A',title:'Corrected'}]));for(const id of ['PG2527','PG2528','PG2561','PG2562'])assert.equal(isCoverageArticle({id}),false)});
