...
test('local accounts require sign-in after setup and enforce administrator provisioning',async()=>{
 ...
 await changeAccess(request,{action:'setup',name:'admin@mcciapune.com',password:'test-password-only-1234'});
 ...
 await assert.rejects(()=>changeAccess(request,{action:'add',name:'viewer@mcciapune.com',password:'test-password-only-1234',role:'Viewer'}),/Administrator/);
 await assert.rejects(()=>changeAccess(request,{action:'login',name:'admin@mcciapune.com',password:'wrong'}),/incorrect/);
 const login=await changeAccess(request,{action:'login',name:'admin@mcciapune.com',password:'test-password-only-1234'});
 const signed=new Request(request,{headers:{cookie:`wt_session=${login.token}`}});
 assert.equal((await identity(signed)).name,'admin@mcciapune.com');
 await assert.rejects(()=>changeAccess(signed,{action:'add',name:'person@example.com',password:'test-password-only-1234',role:'Viewer'}),/mcciapune/);
 await assert.rejects(()=>changeAccess(signed,{action:'add',name:'person@mcciapune.com.evil.com',password:'test-password-only-1234',role:'Viewer'}),/mcciapune/);
 await changeAccess(signed,{action:'add',name:'viewer@mcciapune.com',password:'test-password-only-1234',role:'Viewer'});
